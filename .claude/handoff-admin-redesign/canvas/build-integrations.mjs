...
const CSS = `
    ...
    .grp-hd { display:flex; align-items:baseline; gap:10px; padding:12px 16px 10px; }
    .grp-hd h2 { font-size:13.5px; font-weight:600; }
    .grp-hd p { font-size:12px; color:var(--text-muted); }
    .prov { display:grid; grid-template-columns:38px minmax(0,1fr) 214px 124px; align-items:center;
      column-gap:14px; padding:12px 16px; border-top:1px solid var(--border); }
    .prov:hover { background:var(--stone-50); }
    .prov-logo { width:34px; height:34px; border-radius:9px; display:grid; place-items:center;
      font-size:11.5px; font-weight:700; letter-spacing:.02em; }
    .prov-name { display:grid; gap:2px; min-width:0; }
    .prov-name b { font-size:13.5px; font-weight:600; color:var(--text-strong); }
    .prov-name span { font-size:12px; color:var(--text-muted); }
    .prov-state { display:grid; gap:3px; }
    .prov-state small { font-size:11.5px; color:var(--text-muted); }
    .prov-act { display:flex; justify-content:flex-end; }
    .req { display:grid; grid-template-columns:auto minmax(0,1fr); gap:9px; align-items:start;
      padding:10px 16px; border-top:1px solid var(--border); }
    .req code { font-family:var(--font-mono); font-size:11.5px; color:var(--text-strong);
      background:var(--stone-100); border-radius:4px; padding:1px 5px; }
    .req p { font-size:12px; color:var(--text-muted); margin-top:3px; }
    .banner { display:grid; grid-template-columns:auto minmax(0,1fr) auto; align-items:center; gap:12px;
      padding:12px 16px; border-radius:12px; background:var(--warning-50); border:1px solid #EFDDB6;
      color:var(--warning-600); margin-bottom:16px; }
    .banner b { display:block; font-size:13px; font-weight:600; }
    .banner span { font-size:12.5px; }
`;
...
${prov({ logo: "GW", bg: "var(--warning-50)", fg: "var(--warning-600)", name: "Google Workspace",
...
${prov({ logo: "VB", bg: "var(--stone-100)", fg: "var(--stone-600)", name: "Viber",
...
${prov({ logo: "FB", bg: "var(--sea-50)", fg: "var(--sea-700)", name: "Facebook Page",
...
${prov({ logo: "IG", bg: "var(--sea-50)", fg: "var(--sea-700)", name: "Instagram",
...
${prov({ logo: "GH", bg: "var(--stone-100)", fg: "var(--stone-600)", name: "GitHub",
...
${prov({ logo: "OR", bg: "var(--stone-100)", fg: "var(--stone-600)", name: "OpenRouter",
        ...
        <div style="display:grid; gap:16px">
          ...
          <section class="panel">
            ...
            <div class="req" style="grid-template-columns:auto minmax(0,1fr)">
              ${icon("check", 16)}
              <span><b style="font-size:12.5px">Send replies from the inbox</b><p>Needs Google Workspace or WhatsApp.</p></span>
            </div>
            <div class="req">
              ${icon("check", 16)}
              <span><b style="font-size:12.5px">Hermes translation drafts</b><p>7 listings are waiting on this.</p></span>
            </div>
            <div class="req">
              ${icon("check", 16)}
              <span><b style="font-size:12.5px">Viewing invitations in the calendar</b><p>Needs Google Workspace.</p></span>
            </div>
          </section>

          <section class="panel">
            <div class="panel-hd"><h2>Recent connection events</h2></div>
            <div class="req">${icon("clock", 15)}<span><b style="font-size:12.5px">Google token expired</b><p>28 Aug, 04:12 · delivery paused</p></span></div>
            <div class="req">${icon("clock", 15)}<span><b style="font-size:12.5px">Instagram connected by Mariya</b><p>2 Aug, 11:04</p></span></div>
            <div class="req">${icon("clock", 15)}<span><b style="font-size:12.5px">Typesense schema rebuilt</b><p>29 Jul, 09:30</p></span></div>
          </section>
        </div>
      </div>`;
