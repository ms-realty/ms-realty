...
const LE_CSS = `
    .fbox { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:16px 20px; padding:16px 20px; }
    .fact { display:grid; gap:4px; min-width:0; }
    .fact label { font-size:13px; font-weight:600; color:var(--text-body); display:flex; align-items:center; gap:8px; }
    .src { font-size:11px; font-weight:600; padding:4px 8px; border-radius:var(--r-xs); }
    .src--v { background:var(--success-50); color:var(--success-600); }
    .src--c { background:var(--spring-50); color:var(--spring-800); }
    .src--m { background:var(--warning-50); color:var(--warning-700); }
    .gal2 { display:grid; grid-template-columns:repeat(6,minmax(0,1fr)); gap:8px; padding:16px 20px; }
    .ph2 { height:74px; border-radius:var(--r-sm); background:var(--joint); display:grid; place-items:center;
      color:var(--marble-500); position:relative; }
    ...
    .trow { display:grid; grid-template-columns:34px minmax(0,1fr) auto; gap:12px; align-items:center;
      padding:8px 0; border-bottom:1px solid var(--border); font-size:13px; }
    .trow:last-child { border-bottom:0; }
`;
const LE_BODY = `      <div class="crumbs" style="display:flex; align-items:center; gap:8px; font-size:13px; color:var(--text-muted); margin-bottom:12px">
...
${[
  ...
  ["Furnishing","Not recorded","m","Nobody has confirmed this"],
          ...
          <div class="savebar">
            <span style="font-size:13px" class="muted">A fact marked verified came from the registry, the
          ...
          <div class="gal2">
            ${[1,2,3,4,5,6].map((i) => `<div class="ph2">${icon("image", 22)}${i === 3 ? '<span class="pill pill--warn" style="padding:4px 8px; font-size:11px">Held</span>' : i === 1 ? '<span class="pill pill--ink" style="padding:4px 8px; font-size:11px">Cover</span>' : ""}</div>`).join("")}
          </div>
        </section>
        <div style="display:grid; gap:16px">
          <section class="panel">
            ...
            <div class="side-sect" style="display:grid; gap:8px; font-size:13px">
              <div style="display:flex; gap:8px">${icon("check", 15)}<span>Price, area and location match the source page.</span></div>
              <div style="display:flex; gap:8px">${icon("check", 15)}<span>At least six photos, all reviewed.</span></div>
              <div style="display:flex; gap:8px">${icon("check", 15)}<span>Bulgarian description approved by a person.</span></div>
              <div style="display:flex; gap:8px; color:var(--warning-700)">${icon("alert", 15)}<span>Three facts unconfirmed — allowed, they stay off the page.</span></div>
              <div style="display:flex; gap:8px; color:var(--warning-700)">${icon("alert", 15)}<span>Publishing
...
const TR_CSS = `
    ...
    .diff > div:first-child { border-right:1px solid var(--border); background:var(--tile); }
    .diff h4 { font-size:11px; font-weight:600; color:var(--text-muted); margin-bottom:8px;
      display:flex; align-items:center; justify-content:space-between; }
    .diff p { font-size:13px; line-height:1.6; color:var(--text-body); }
    .fact-lock { display:inline; background:var(--spring-50); border-bottom:1px solid var(--spring-200);
      padding:0 4px; font-weight:600; color:var(--spring-800); }
    .qrow { display:grid; grid-template-columns:auto minmax(0,1fr); gap:12px; align-items:center;
      padding:12px 16px; border-bottom:1px solid var(--border); }
    .qrow:last-child { border-bottom:0; }
    .qrow:hover { background:var(--tile); }
`;
...
${[
  ...
  ["Purchase fees guide","/rakovodstva/taksi · EL", false],
            ...
            <span style="min-width:0"><b style="font-size:13px; font-weight:600; display:block; color:var(--text-strong)">${t}</b>
              <span style="font-size:11px" class="muted">${m}</span></span>
            </div>`).join("\n")}
        ...
        <section class="panel">
          ...
          <div class="diff">
            <div>
              ...
              <p style="margin-top:12px; font-size:11px" class="muted">Editing the source invalidates every
            ...
            <div>
              ...
              <div style="margin-top:12px; display:grid; gap:8px; font-size:13px">
                <div style="display:flex; gap:8px; color:var(--success-600)">${icon("check", 14)}<span>Every fact matches the source, field by field.</span></div>
                <div style="display:flex; gap:8px; color:var(--success-600)">${icon("check", 14)}<span>No claim was added that the Bulgarian does not make.</span></div>
                <div style="display:flex; gap:8px; color:var(--success-600)">${icon("check", 14)}<span>SEO title 54 characters, description 141.</span></div>
              </div>
            </div>
          </div>
          <div class="savebar"><span style="font-size:13px" class="muted">Approving puts your name on this text.
