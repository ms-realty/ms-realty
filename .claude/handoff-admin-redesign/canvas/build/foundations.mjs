...
const CSS = `
    ...
    .doc-hd p { margin-top:4px; font-size:13px; color:var(--text-muted); max-width:640px; }
    ...
    .ramp { display:grid; grid-template-columns:96px repeat(11,minmax(0,1fr)); align-items:center; gap:0;
      margin-bottom:8px; }
    .ramp b { font-size:13px; font-weight:600; color:var(--text-strong); }
    .swatch { height:46px; display:grid; align-items:end; padding:4px 4px; font:500 11px var(--font-mono); }
    .roles { width:100%; }
    .roles td, .roles th { padding:8px 12px; font-size:13px; }
    .chipbox { display:inline-flex; align-items:center; gap:8px; }
    ...
    .scale { display:flex; align-items:flex-end; gap:12px; }
    .scale div { background:var(--spring-100); border-radius:var(--r-edge) var(--r-edge) 0 0; }
    .scale span { display:block; margin-top:4px; font:500 11px var(--font-mono); color:var(--text-muted);
      text-align:center; }
    .rad { display:flex; gap:16px; }
    .rad div { width:74px; height:56px; background:var(--joint); border:1px solid var(--border-control);
      display:grid; place-items:end center; padding-bottom:4px; font:500 11px var(--font-mono);
    ...
    .elev div { width:120px; height:64px; border-radius:var(--r-lg); background:var(--surface);
      border:1px solid var(--border); display:grid; place-items:center; font:500 11px var(--font-mono);
      color:var(--text-muted); }
    .icons { display:grid; grid-template-columns:repeat(14,1fr); gap:4px; }
    ...
    .gridfig .bar { height:22px; border-radius:var(--r-sm); background:var(--sunken); display:grid;
      place-items:center; font:500 11px var(--font-mono); color:var(--text-muted); }
    .g12 { display:grid; grid-template-columns:repeat(12,minmax(0,1fr)); gap:20px; }
    .g12 i { background:var(--spring-50); border:1px dashed var(--spring-200); border-radius:var(--r-edge); }
    .note-b { font-size:13px; color:var(--text-muted); margin-top:8px; }
    .rtl { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:16px; }
    .rtl .c { padding:12px 16px; border:1px solid var(--border); border-radius:var(--r-md);
      background:var(--surface); }
    .rtl .c b { display:block; font-size:13px; margin-bottom:8px; }
`;
...
const ROLES = [
  ...
  ["--spring-800", "#183B39", "Neutral informational state", "#ECF3F2", "text"],
];
...
const TYPE = [
  ...
  ["Display / 26", "font-family:var(--font-display); font-size:27px; font-weight:600; letter-spacing:-.015em", "Page titles only", "Lead inbox"],
  ...
  ["UI / 14.5 semi", "font-size:16px; font-weight:600", "Panel headings", "Needs you now"],
  ["UI / 13.5", "font-size:13px", "Default body and rows", "Two-bedroom apartment with a south terrace"],
  ["UI / 13 semi", "font-size:13px; font-weight:600", "Row primaries, field labels", "Reply deadline"],
  ["UI / 12.5", "font-size:13px", "Table cells, secondary rows", "Sandanski · Apartment · €68,000"],
  ["UI / 12", "font-size:13px; color:var(--text-muted)", "Meta, hints, captions", "Escalated to manager 4 Jul, 04:00"],
  ["UI / 11.5 semi", "font-size:11px; font-weight:600", "Pills and chips", "Needs review"],
  ["Mono / 11.5", "font-family:var(--font-mono); font-size:11px; color:var(--text-muted)", "References, ids, env keys", "MS-00815 · HERMES_API_KEY"],
];
...
<div class="grp">
  ...
  <div class="ramp" style="margin-top:12px">
    <b>Status</b>
...
${TYPE.map(([n, s, note, sample]) => `      <div class="type-row">
        <span><b style="font-size:13px; display:block">${n}</b><span class="m">${note}</span></span>
    ...
    <div>
      <div class="panel" style="padding:16px">
        ...
        <div class="scale">
          <div style="width:32px; height:20px"></div><div style="width:38px; height:24px"></div>
        ...
        <div style="display:flex; gap:12px; margin-top:4px">
          ${TYPE_SIZES.map((s) => `<span style="width:38px; font:500 11px var(--font-mono); color:var(--text-muted); text-align:center">${s}</span>`).join("")}
        </div>
      ...
      <div class="panel" style="padding:16px; margin-top:16px">
        <b style="font-size:12px">Prices and references keep their own faces</b>
        <div style="display:flex; align-items:baseline; gap:16px; margin-top:8px">
          <span class="price" style="font-size:20px">€185,000</span>
          <span class="price" style="font-size:12px">€400 / month</span>
          <span class="mono" style="font-size:12px">MS-00191</span>
        </div>
...
<div class="grp">
  ...
  <div class="cols" style="grid-template-columns:1.1fr .9fr">
    <div class="panel" style="padding:16px">
      <b style="font-size:12px">Spacing — a 4px unit, ${SPACING_STEPS.length} steps</b>
      <div class="scale" style="margin-top:12px">
        ${SPACING_STEPS.map((v) => `<div style="width:${v}px; height:${v}px; background:var(--spring-100)"></div>`).join("")}
      </div>
      <div style="display:flex; gap:12px; margin-top:4px">
        ${SPACING_STEPS.map((v) => `<span style="width:${v}px; font:500 11px var(--font-mono); color:var(--text-muted)">${v}</span>`).join("")}
      </div>
      ...
      <b style="font-size:13px; display:block; margin-top:20px">Radius — five, and one rule each</b>
      <div class="rad" style="margin-top:12px">
        <div style="border-radius:var(--r-edge)">4 chip</div>
        <div style="border-radius:var(--r-panel)">6 inner</div>
        <div style="border-radius:var(--r-panel)">8 control</div>
        <div style="border-radius:var(--r-panel)">14 panel</div>
        <div style="border-radius:var(--r-pill)">999 pill</div>
      </div>
    </div>
    <div class="panel" style="padding:16px">
      <b style="font-size:12px">Elevation — two, and one of them is nothing</b>
      <div class="elev" style="margin-top:12px">
        <div style="box-shadow:var(--e-rest)">rest — a grout line, no shadow</div>
        <div style="box-shadow:var(--e-float)">float — over the page, traps focus</div>
      </div>
      <p class="note-b">A panel at rest is separated by the grout line between it and the tile, never by a
        shadow. Only what floats over the page casts one: a menu, a dialog, a toast. The older three-step
        names still resolve, and two of them resolve to nothing.</p>
      <b style="font-size:12px; display:block; margin-top:20px">Motion</b>
      <p class="note-b" style="margin-top:8px">140ms ease-out on hover and state, 200ms on anything entering
    ...
    <div>
      <div class="gridfig">
        ...
        <div class="b">
          ...
          <span style="border:4px dashed var(--border-control); border-radius:var(--r-md); display:grid; place-items:center; font:500 11px var(--font-mono); color:var(--text-muted)">content · 26px gutter</span>
        </div>
      </div>
...
<div class="grp" style="margin-bottom:0">
  ...
  <div class="rtl">
    ...
    <div class="c" dir="rtl">
      ...
      <p style="font-size:13px; color:var(--text-muted)">Hebrew mirrors the whole shell: the rail moves right,
