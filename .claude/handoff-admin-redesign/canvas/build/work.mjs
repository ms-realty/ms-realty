...
const VIEW_CSS = `
    ...
    .wk-hd { padding:9px 8px; border-bottom:1px solid var(--border); border-right:1px solid var(--border);
      background:var(--stone-50); font-size:11.5px; font-weight:600; color:var(--text-muted); text-align:center; }
    .wk-hd b { display:block; font-family:var(--font-display); font-size:16px; color:var(--text-strong); }
    .wk-hd[data-today] { background:var(--sea-50); color:var(--sea-700); }
    .wk-t { border-right:1px solid var(--border); border-bottom:1px solid var(--border); padding:4px 5px;
      font:500 10.5px var(--font-mono); color:var(--text-muted); text-align:right; }
    .wk-c { border-right:1px solid var(--border); border-bottom:1px solid var(--border); min-height:52px; padding:3px; }
    .ev { display:grid; gap:2px; padding:5px 7px; border-radius:var(--r-sm); font-size:11.5px; margin-bottom:3px;
      border-left:3px solid; }
    .ev b { font-weight:600; font-size:11.5px; }
    ...
    .ev--sea { background:var(--sea-50); border-color:var(--sea-600); color:var(--sea-700); }
    .ev--busy { background:var(--stone-100); border-color:var(--stone-400); color:var(--stone-700); }
      ...
      <div style="display:grid; grid-template-columns:minmax(0,1fr) 316px; gap:20px; align-items:start">
        ...
        <div class="side">
          <section class="panel">
            ...
            <div class="sect">
              <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:7px">
                <b style="font-size:13px">Georgi Nikolov · viewed 30 Aug</b><span class="pill pill--sea"><i></i>Feedback due</span>
              </div>
              <p style="font-size:12.5px; color:var(--text-muted)">A follow-up call was promised within three days.</p>
              <div style="display:flex; gap:8px; margin-top:10px">
          ...
          <section class="panel">
            <div class="panel-hd"><h2>Availability</h2><a href="#" style="font-size:12.5px; font-weight:600">Edit</a></div>
            <div class="sect" style="display:grid; gap:9px">
              <div style="display:flex; justify-content:space-between; font-size:12.5px"><span>Mariya Ruseva</span><span class="muted">Mon–Fri 09:00–18:00</span></div>
              <div style="display:flex; justify-content:space-between; font-size:12.5px"><span>Petar Dimitrov</span><span class="muted">Mon–Sat 10:00–19:00</span></div>
...
const TASK_CSS = `
    ...
    .tk:hover { background:var(--stone-50); }
    .tk-main { display:grid; gap:2px; min-width:0; }
    .tk-main b { font-size:13.5px; font-weight:600; color:var(--text-strong); }
    .tk-meta { display:flex; gap:8px; font-size:12px; color:var(--text-muted); min-width:0; }
    .tk-meta span { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    .day { padding:9px 16px; background:var(--sunken); font-size:11.5px; font-weight:600; color:var(--text-muted);
...
function task({ done, title, meta, ctx, when, tone = "", who, ai }) {
  return `        <div class="tk">
          ...
          <span style="font-size:12px" class="muted">${ctx}</span>
          <span style="font-size:12.5px; font-weight:600; ${tone}">${when}</span>
          <span style="display:flex; justify-content:flex-end; gap:6px; align-items:center">${ai ? `<span class="pill pill--ai">${icon("sparkles", 11)}</span>` : ""}<span class="av">${who}</span></span>
        </div>`;
}
...
const REQ_CSS = `
    ...
    .rq h3 { font-size:13.5px; font-weight:600; }
    .rq p { font-size:12.5px; color:var(--text-muted); margin-top:3px; }
    .rq-act { display:flex; flex-direction:column; gap:7px; align-items:stretch; }
`;
const REQ_BODY = `      <div class="ph">
      ...
      <section class="panel">
        ...
        <div class="rq">
          <div><span class="pill pill--warn"><i></i>Language request</span><p class="mono" style="margin-top:6px">/fr/</p></div>
          <div><h3>French was requested three times this week</h3>
            ...
            <div style="display:flex; gap:6px; margin-top:9px"><span class="pill pill--sand">3 visitors</span>
        ...
        <div class="rq">
          <div><span class="pill pill--warn"><i></i>Saved search alert</span><p class="mono" style="margin-top:6px">alert-0042</p></div>
          <div><h3>A saved search has matched nothing for 21 days</h3>
            ...
            <div style="display:flex; gap:6px; margin-top:9px"><span class="pill pill--sand">Sofia Marinova</span>
      ...
      <section class="panel" style="margin-top:16px">
        <div class="panel-hd"><h2>Answered recently</h2><a href="#" style="font-size:12.5px; font-weight:600">All 14</a></div>
        <table>
...
const CON_BODY = `      <div class="ph">
      ...
      <section class="panel">
        ...
        <table>
          <thead><tr><th style="width:34px"></th><th>Person</th><th>Role</th><th>Company</th><th>Language</th><th>Consent</th><th>Open work</th><th>Broker</th><th>Last contact</th><th></th></tr></thead>
          <tbody>
${[
  ...
  ["DV","Dmitri Volkov","d.volkov@••••.ru","Buyer","sea","—","RU","withdrawn","1 enquiry","sand","PD","yesterday"],
              ...
              <td><span style="display:flex; align-items:center; gap:10px"><span class="av">${ini}</span>
      ...
      <div style="display:grid; grid-template-columns:minmax(0,1fr) 316px; gap:20px; align-items:start">
        ...
        <div style="display:grid; gap:16px">
          <section class="panel">
            ...
            <div class="sect" style="display:grid; gap:9px; font-size:12.5px; color:var(--text-body)">
              <div style="display:flex; gap:9px">${icon("check", 15)}<span>Removes the person from every saved-search alert and campaign audience.</span></div>
              <div style="display:flex; gap:9px">${icon("check", 15)}<span>Keeps the records the agency is legally required to hold for a transaction.</span></div>
              <div style="display:flex; gap:9px">${icon("check", 15)}<span>Writes an entry naming who withdrew it and when, which cannot be edited.</span></div>
              <div class="note note--info" style="margin-top:3px">${icon("alert", 14)}<span>Reply to a direct enquiry is a separate purpose and is not removed by withdrawing marketing consent.</span></div>
            </div>
          </section>
          <section class="panel">
