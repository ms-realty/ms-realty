...
**Blocked until launch:** **legacy media migration** (`npm run media:mirror` — 1,714 `/wp-content/uploads` assets are still hotlinked from the old WordPress origins and 404 the moment DNS moves), **locale content parity** (EN/DE/NL are marked indexable but have zero approved listing translations, so the rebuild currently ships fewer languages than the legacy site), real external SEO exports (Search Console / Yandex / backlinks) under
