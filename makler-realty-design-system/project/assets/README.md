...
- **`fonts/`** — licensed brand webfonts (`.woff2`). If added, replace the Google-Fonts `@import` in `../tokens/fonts.css` with `@font-face` rules pointing here. Current fonts are stand-ins (Source Serif 4 / Commissioner / IBM Plex Mono plus Noto Hebrew fallbacks — Latin + Cyrillic + Greek + Hebrew).
