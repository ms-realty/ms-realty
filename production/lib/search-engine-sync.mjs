...
async function queryPostgres({
  ...
  const drizzle = runtime?.db?.drizzle;
  if (typeof drizzle?.execute !== "function") throw new Error("Payload Postgres runtime does not expose drizzle.execute");
  ...
  const pageStatement = sql`
    ...
    OFFSET ${offset}
  `;
  const total = postgresResultCount(postgresRows(await drizzle.execute(countStatement)));
  const rows = postgresRows(await drizzle.execute(pageStatement));
...
async function readPostgresProjectionDocuments({ postgres = {} } = {}) {
  ...
  const rows = postgresRows(
    await drizzle.execute(sql`SELECT * FROM ${sql.raw(`"public"."${POSTGRES_PUBLIC_SEARCH_VIEW}"`)} ORDER BY "id" ASC`),
  );
