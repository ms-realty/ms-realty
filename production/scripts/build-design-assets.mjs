...
const [publicJs, adminJs] = [PUBLIC_APP_JS, ADMIN_APP_JS].map(source => transformSync(source, { minify: true, target: "es2020" }).code.trim());
const publicClientHash = createHash("sha256").update(publicJs).digest("hex").slice(0, 12);
const adminClientHash = createHash("sha256").update(adminJs).digest("hex").slice(0, 12);
...
writeFileSync(path.join(VENDOR_DIR, "ms-realty-public.js"), `${header}${publicJs}\n`);
writeFileSync(path.join(VENDOR_DIR, "ms-realty-admin.js"), `${header}${adminJs}\n`);
