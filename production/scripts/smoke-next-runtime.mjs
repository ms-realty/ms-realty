...
const child = spawn(
  process.execPath,
  [fromRoot("node_modules", "next", "dist", "bin", "next"), "start", "-H", "127.0.0.1", "-p", String(port)],
  {
    cwd: fromRoot(),
    env: {
      ...process.env,
...
try {
  ...
  assert(search.status === 200, "Next localized search did not render");
  assert(search.headers.get("content-type")?.startsWith("text/html"), "Next localized search must render HTML");
  ...
  console.log(
    JSON.stringify({
      ...
      checked: ["health", "readiness", "home", "search", "vendor", "legacy_redirect"],
    }),
  );
} finally {
  await stop(child);
