...
test("HTML renderer emits SEO-safe listing, search, and fallback documents", () => {
  ...
  // Public pages keep their own stylesheet and load the pinned font subsets.
  assert.doesNotMatch(homeHtml, /ms-realty-admin\.css/);
  assert.match(homeHtml, /href="\/vendor\/ms-realty-fonts\.css\?v=[a-f0-9]{12}"/);
...
test("admin lead values are localized without exposing raw workflow codes", () => {
  ...
  assert.match(ADMIN_APP_JS, /if \(form\.hasAttribute\("data-locale-form"\)\) window\.location\.reload\(\)/);
  assert.doesNotMatch(html, />website_listing_detail</);
  assert.doesNotMatch(html, />manager escalation required</);
});

test("admin lead inbox omits empty viewing and seller queues until there is actionable work", () => {
  ...
  assert.doesNotMatch(html, /data-lead-secondary-queues="true"/);
  assert.doesNotMatch(html, /data-empty-viewing-follow-ups="true"/);
  assert.doesNotMatch(html, /data-empty-seller-pipeline="true"/);
});
