...
test("production recovery import validates before persisting and exposes the safe template", (t) => {
  ...
  // Same reason as the HTTP test: pin the clock to the fixture, not the calendar.
  assert.equal(
    productionRecoveryState(reportPath, { now: "2026-07-23T01:00:00.000Z", publicKey: RECOVERY_PUBLIC_KEY }).status,
    "pass",
  );
